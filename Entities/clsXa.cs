...
namespace MDSolution
{
    public class clsXa
    {
		...
		public void Save(OleDbConnection cn, OleDbTransaction trans)
		{
			...
			{
                //id = lddata.DBModule.GetNewID(typeof(Xa), "tbl_Xa", cn, trans);
				ID = DBModule.GetNewID(typeof(clsXa), "tbl_Xa", cn, trans);
				//NgayTao = DateTime.Now;        
				//NgaySua = DateTime.Now;        
				// build SQL statement
                strSQL = "Insert into tbl_Xa" +
				"(ID,Ten,HuyenID,CumID,DinhMuc,MaXa,CuaDonVi) Values(" +
					ID.ToString()+","+"N'" + DBModule.RefineString(Ten) + "'"+","+HuyenID.ToString()+","+CumID.ToString()+","+DinhMuc.ToString()+","+"N'" + DBModule.RefineString(MaXa) + "'"+","+CuaDonVi.ToString()+")";
			}
			else // edit object, we update old record in database
			{
                ...
                strSQL = "Update tbl_Xa set " +
					"Ten="+"N'" + DBModule.RefineString(Ten) + "'"+","+"HuyenID="+HuyenID.ToString()+","+"CumID="+CumID.ToString()+","+"DinhMuc="+DinhMuc.ToString()+","+"MaXa="+"N'" + DBModule.RefineString(MaXa) + "'"+","+"CuaDonVi="+CuaDonVi.ToString()+
			...
			/*
			if( ID <= 0 )
				ID = long.Parse(DBModule.ExecuteQueryGetOneResult("SELECT Max(ID) FROM tbl_Xa", cn, trans));
				*/
		}
        ...
        {           
            ...
            //    strSQL = strSQL + " Order By b.ThuTu, a.ID ";// +DBModule.RefineString(OrderBy);

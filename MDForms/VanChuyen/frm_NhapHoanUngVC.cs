...
{
    public partial class frm_NhapHoanUngVC : Form
    {
        public long OK = 0;
        ...
        {
            string stt = DBModule.ExecuteQueryForOneResult("Get_MaPhieu 'tbl_HoanUngVanChuyen', " + MDSolutionApp.VuTrongID.ToString(), null, null);
        ...
        public frm_NhapHoanUngVC(long HopDongVanChuyen_ID, bool Add = true)//Nhập mới
        {
            InitializeComponent();
        ...
        public frm_NhapHoanUngVC(long HopDongVanChuyen_ID)//Sửa
        {
            InitializeComponent();
            clsHopDongVanChuyen objHDVC = new clsHopDongVanChuyen(HopDongVanChuyen_ID);
            objHDVC.Load(null, null);
            ...
            if (Valid_Form())
            {
                if (HoanUngID < 0)//Nếu là nhập mới
                {
                    try
                    {
                        ...
                        clsHoanUngVanChuyen objHUVC = new clsHoanUngVanChuyen();
                        ...
                        MessageBox.Show("Bạn đã thêm mới mục hoàn ứng thành công!", "SLS", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        OK = 1;
                    }
                    catch (Exception ex)
                    {
                        MessageBox.Show("Có lỗi khi thêm mới mục hoàn ứng !\n" + ex.ToString(), "SLS", MessageBoxButtons.OK, MessageBoxIcon.Hand);
                        return;
                    }
                }
                else
                {
                    try
                    {
                        ...
                        objHUVC.XeID = XeVCID;
                        objHUVC.Save(null, null);   
                        MessageBox.Show("Bạn đã sửa thông tin mục hoàn ứng thành công!", "SLS", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        OK = 1;
                    }
                    catch
                    {
        ...
        private void frm_NhapHoanUngVC_Load(object sender, EventArgs e)
        {
